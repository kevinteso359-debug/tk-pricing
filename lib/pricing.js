...
export const DEFAULT_PLATFORMS = [
  {
    ...
    name: 'eBay',
    feePct: 0.1435,
    paymentPct: 0, // ✅ FIX: eBay ha già il pagamento incluso
    fixedFee: 0.35,
    targetMarginPct: 0.22,
    shippingSubsidy: 0,
    roundStep: 1,
    ending: 0.99 // più realistico
  },
  {
    key: 'cardmarket',
    name: 'Cardmarket',
...
//
// 🔥 FIX PRINCIPALE QUI
// Usa SEMPRE EUR se disponibile
//
export function getSourceCostEur(product, settings) {
  const eur = toNumber(product?.costs?.EUR);

  // ✅ PRIORITÀ ASSOLUTA EUR
  if (eur > 0) return eur;

  const currency = settings.sourceCurrency;

  if (currency === 'JPY') {
    return toNumber(product?.costs?.JPY) * toNumber(settings.fxJpyToEur);
  }

  if (currency === 'USD') {
    return toNumber(product?.costs?.USD) * toNumber(settings.fxUsdToEur);
  }

  if (currency === 'SGD') {
    return toNumber(product?.costs?.SGD) * toNumber(settings.fxSgdToEur);
  }

  if (currency === 'AUD') {
    return toNumber(product?.costs?.AUD) * toNumber(settings.fxAudToEur);
  }
