...
// Usa SEMPRE JPY e converte automaticamente in EUR
export function getSourceCostEur(product, settings) {
  const jpy = toNumber(product?.costs?.JPY);
  const fxJpyToEur = toNumber(settings?.fxJpyToEur);

  if (jpy <= 0 || fxJpyToEur <= 0) return 0;

  return jpy * fxJpyToEur;
}
